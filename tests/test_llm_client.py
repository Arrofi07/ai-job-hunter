...
def test_routes_structured_task_to_configured_provider(monkeypatch):
    settings.llm_provider_structured = LLMProvider.GEMINI
    gemini = MagicMock()
    gemini.generate.return_value = "structured response"
    client = _client_with_mocked_providers(monkeypatch, {LLMProvider.GEMINI: gemini})
...
#@pytest.mark.skip(reason="Requires real API keys in .env — run manually, not in CI")
